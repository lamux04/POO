...
int main(int argc, const char **argv)
{
  ...
  if (c1.findClass({ "Articulo", "ArticuloAlmacenable", "Autor",
    ...
    "class std::basic_ostream<char> &"} }, "Libro", { "const" },
      ...
      " del método impresion_especifica en cada clase.");
    c1.methodWithReferencedMemberVariable({ "impresion_especifica" }, { {
    "class std::basic_ostream<char> &"} }, "Revista", { "const" },
      { "numero_" }, "Revisa qué debe imprimir la definición"
      " del método impresion_especifica en cada clase.");
    c1.methodWithReferencedMemberVariable({ "impresion_especifica" }, { {
    "class std::basic_ostream<char> &"} }, "Revista", { "const" },
      { "periodicidad_" }, "Revisa qué debe imprimir la definición"
      " del método impresion_especifica en cada clase.");
    c1.methodWithReferencedMemberVariable({ "impresion_especifica" }, { {
    ...
    c1.method({ "nombre","apellidos","direccion" }, { {},{},{} }, "Autor",
      { "const","const","const" }, { true, true, true },
    ...
    " 'ArticuloAlmacenable', 'Autor', 'Libro', 'Revista' o 'LibroDigital'\n";
#endif // P4
  ...
  if (c5.findClass({ "Pedido_Articulo", "LineaPedido" })) {
    ...
    */
    ...
    {"class Articulo &", "class Pedido &", "double",
        "unsigned int"} }, "Pedido_Articulo", { "noconst","noconst" }, { true, true },
  ...
  if (c6.findClass({ "Usuario_Pedido" })) {
    ...
    c6.methodWithReferencedMethod({ "asocia" }, { {"?"} }, "Usuario_Pedido", { "?" },
      { "asocia" }, { {"?"} }, "Usuario_Pedido", { "?" },
      "El método asocia debe encargarse de ambas direcciones de la asociación.");
